...
class FPCoreParser(Parser):
    ...
    def operation(self, p):
        if p[1] == "/":
            assert(len(p.expr_plus) == 2)
            return ast.Operation("*", p.expr_plus[0],
                                 ast.Operation("inv", p.expr_plus[1]))
    ...
    def property(self, p):
        return ast.Property(p.symbol, p[2])
...
def main(argv):
    ...
    for fpc in parse(text):
        logger.blog("FPCore", repr(fpc))

    logger(" Lexing time: {:.6f} msec", fpcore_lexer.timer.elapsed())
