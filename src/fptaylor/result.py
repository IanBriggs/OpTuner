...
from fptaylor.parser import parse
...
class Result():
            ...
            with subprocess.Popen(shlex.split(run_command),
                ...
                if self.stderr != "":
                    for line in self.stderr.splitlines():
                        if (("Large second-order error" in line
                             or "Try intermediate-opt" in line)
                            and self.config["--intermediate-opt"] == "true"):
                            continue
    ...
    def _scrape_output(self):
        ...
        def peek_grab(pre):
            ...
            line = lines[idx]
            logger.llog(Logger.EXTRA, "Mathing prefix '{}' with '{}'", pre, line)
        ...
        def grab_taylor_forms(expressions):
            ...
            while True:
                line = lines[idx]
                logger.llog(Logger.EXTRA, "Matching taylor expression in '{}'", line)
                parts = line.split()
        ...
        def grab_original_expressions(expressions):
            ...
            while True:
                line = lines[idx]
                logger.llog(Logger.EXTRA, "Matching source expression in '{}'", line)
                parts = line.split()
                index = parts[0].rstrip(":")
                try:
                    int(index)
        ...
        def grab_delta_terms(deltas):
            ...
            while True:
                line = lines[idx]
                logger.llog(Logger.EXTRA, "Matching delta value in '{}'", line)
                parts = line.split()
                index = parts[0].rstrip(":")
                try:
                    int(index)
        ...
        if try_grab("Simplifying Taylor forms...") != None:
            ...
            if try_grab("Computing absolute errors") is not None:
                ...
                self.result["absolute_errors"]["m1"] = grab_answer("exact bound ")
            ...
            if try_grab("Computing relative errors") is not None:
                ...
                if try_grab("Solving the exact optimization problem") is not None:
                    self.result["relative_errors"]["m1"] = grab_answer("exact bound-rel ")
                    self.result["relative_errors"]["m2"] = grab("total2:")
                    self.result["relative_errors"]["total"] = grab("exact total-rel:")
                else:
                    self.result["relative_errors"]["m1"] = None
                    self.result["relative_errors"]["m2"] = None
                    self.result["relative_errors"]["total"] = None
    ...
    def _parse_result(self):
        expressions = self.result["expressions"]
        if len(expressions) > 0:
            for idx in expressions.keys():
                string = expressions[idx]["taylor_expression"]
                logger.llog(Logger.EXTRA, "Parsing expression: {}", string)
                expressions[idx]["taylor_expression"] = parse(string)

                string = expressions[idx]["source_expression"]
                logger.llog(Logger.EXTRA, "Parsing expression: {}", string)
                expressions[idx]["source_expression"] = parse(string)
