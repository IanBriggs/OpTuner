 ...
 *---------------------------------------------------------------------------
 * $File: //depot/povray/spec-3.6/source/frontend/messageoutput.cpp $
 * $Revision: #1 $
 * $Change: 5014 $
...
void MessageOutput::Printf(int stream, const char *format, ...)
{
	...
	if((streams[ALL_STREAM] != NULL) && (stream != STATUS_STREAM))
...
void MessageOutput::RenderOptions(POVMSObjectPtr msg, POVMSObjectPtr, int)
{
...
#else
		if(toupper(outputFormat) == 'J')
			Printf(RENDER_STREAM, "  Output file: %s, %d bpp, quality %d%s%s %s\n", charbuf, outputQuality, outputCompression, "%", al, t);
		else
			Printf(RENDER_STREAM, "  Output file: %s, %d bpp%s %s\n", charbuf, outputQuality, al, t);
#endif /* SPEC */
	}
	...
	if(b == true)
	{
		...
		b = false;
		(void)POVMSUtil_GetBool(msg, kPOVAttrib_AntialiasClipped, &b);
		Printf(RENDER_STREAM, "  Antialiasing.........On\n",
	              (int)i, (float)f, (int)i2, (float)f2);
		if(b == true)
			Printf(RENDER_STREAM, "  AA Options: (Method %d, Threshold %.3f, Depth %d, Jitter %.2f, Clipped)\n",
	              (int)i, (float)f, (int)i2, (float)f2);
		else
			Printf(RENDER_STREAM, "  AA Options: (Method %d, Threshold %.3f, Depth %d, Jitter %.2f, Unclipped)\n",
	              (int)i, (float)f, (int)i2, (float)f2);
	}
	else
