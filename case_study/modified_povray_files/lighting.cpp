  ...
  while(n<photonOptions.minGatherCount && step<photonOptions.photonMap.gatherNumSteps)
  {
    ...
    for(j=0; j<tempn; j++)
    {
      ...
      */
      ...
      Light_Source_Ray.Direction[Y] = SIN_THETA(theta); // photonOptions.sinTheta[theta];
      Light_Source_Ray.Direction[X] = COS_THETA(theta); // photonOptions.cosTheta[theta];

      Light_Source_Ray.Direction[Z] = Light_Source_Ray.Direction[X]*SIN_PHI(phi); // photonOptions.sinTheta[phi];
      Light_Source_Ray.Direction[X] = Light_Source_Ray.Direction[X]*COS_PHI(phi); // photonOptions.cosTheta[phi];
